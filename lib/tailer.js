...
class Tailer {
   ...
   */
  constructor (config, customUtils, cliMode) {
    const opts = {
      prefix: config.prefix || (config.uniqueId ? config.uniqueId + '_' : ''),
      uniqueId: config.uniqueId || config.prefix || '',
    ...
    this.url2 = this.url.replace(/\/(\w|-|_)+(\?|$)/, '/local$2')
    this.dbName = this.url.split(/\/|\?/)[3]
    this.defs = createDefs(config.collections, this.dbName, opts)
    this.lastTs = 0
    this.mysql = new MySQL(config.dist, this.defs, opts, this.customUtils)
  }
   ...
   */
  importAll () {
    util.log(`[${this.dbName}] Begin to import...`)
  ...
  importCollection (def) {
    util.log(`[${this.dbName}] Import records in ${def.ns} ...`)
    return new Promise(resolve =>
      mongodb.connect(this.url, { 'autoReconnect': true })
        .then(db => {
  ...
  updateTimestamp (ts, skipUpdateMySQL) {
    ...
    return new Promise(resolve =>
      mongodb.connect(this.url2, { 'autoReconnect': true })
        .then(db =>
  ...
  tail () {
    ...
    util.log(`[${this.dbName}] Begin to watch at "${this.dbName}" ... (from ${ts})`)
    return new Promise((resolve, reject) =>
      mongodb.connect(this.url2).then(db => {
        ...
        stream
          ...
          .on('close', () => {
            util.log('[${this.dbName}] Stream closed....')
            this.db = null
            db.close()
            resolve()
          })
